...
inline void DrawX(IplImage* img, int x, int y, int r = 8, const CvScalar& color = CV_RGB(0, 255, 0), int t = 3)
{
    cvLine(img, cvPoint(x - r, y - r), cvPoint(x + r, y + r), color, t, 8, 0);
    cvLine(img, cvPoint(x + r, y - r), cvPoint(x - r, y + r), color, t, 8, 0);
}

inline void DrawCircle(IplImage* img, int x, int y, int r = 8, const CvScalar& color = CV_RGB(0, 255, 0), int t = 3)
{
    cvCircle(img, cvPoint(x, y), r, color, t, 8, 0);
}

inline void DrawLine(IplImage* img, const util::_point& pt1, const util::_point& pt2, const CvScalar& color = CV_RGB(0, 255, 0), int t = 2.5)
{
    cvLine(img, cvPoint(pt1.x, pt1.y), cvPoint(pt2.x, pt2.y), color, t);
}
...
static void DrawFeature(IplImage* img, const util::feature& feat)
{
    ...
    cvLine(img, start, end, color, 1, 12, 0);
    cvLine(img, end, h1, color, 1, 12, 0);
    cvLine(img, end, h2, color, 1, 12, 0);
}
...
inline void ConvertGrayMRC2RGB(const IplImage* src, IplImage** dst)
{
	
	IplImage* slice = cvCloneImage(src), * init;
	util::ConvertTo1(slice, true);
	util::CopyToUChar256(slice, &init, 255, 0, true);
	cvReleaseImage(&slice);
	*dst = cvCreateImage(cvSize(init->width, init->height), IPL_DEPTH_8U, 3);
	cvCvtColor(init, *dst, CV_GRAY2RGB);
	cvReleaseImage(&init);
}
